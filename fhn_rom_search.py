# new_rom.py
"""Do the hyperparameter search for a new data set."""
import os
import logging
import numpy as np
...
def generate_data(label, numchunks=None):
    ...
    if numchunks is not None:
        for chunk in np.split(training_parameters, numchunks, axis=0):
            solver.add_snapshot_sets(chunk)
            solver.save(dm.solverfile, overwrite=True)
    else:
        solver.add_snapshot_sets(training_parameters)
        solver.save(dm.solverfile, overwrite=True)
...
def main(label, rs, timelimit=12, fromregsfile=False, µ_test=None):
    ...
    if fromregsfile:
        ...
        rom = solver.train_rom(rs, ROMClass=fhn.AffineFHNROM,
                               bases=label, trialtimelimit=timelimit,
                               regguess=λs, µ_test=µ_test)
    else:
        rom = solver.train_rom(rs, ROMClass=fhn.AffineFHNROM,
                               bases=label, trialtimelimit=timelimit,
