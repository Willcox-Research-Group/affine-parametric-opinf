...
def imex(t, u0, A, nonlin, compute_du=False, sparse_lu=False):
    """Solve the system

        du / dt = Au(t) + nonlin(u(t), t),    u(0) = u0,    t[0] ≤ t ≤ t[-1],

    via a first-order implicit-explicit (IMEX) method:

        (I - δt*A)u_{k+1} = u_{k} + δt*nonlin(u_{k}).
    ...
    A : (n, n) ndarray
        State matrix.
    nonlin : callable(u(t), t)
        Function to calculate the nonlinear terms in the state equation.
    compute_du : bool
        If True, compute and return time derivatives in addition to the state.
    sparse_lu : bool
    ...
    n = len(u0)
    if A.shape != (n, n):
        raise ValueError(f"{A.shape} = A.shape but u0.shape = {u0.shape}")

    # Get the (uniform) time step.
    dt = t[1] - t[0]
    # assert np.allclose(np.diff(t), dt)

    # Allocate space for the solutions.
    u = np.empty((n, k))
    u_nonlin = np.empty((n, k))
    u[:, 0] = u0.copy()
    ...
    if sparse_lu:
        ...
        def _linsolve(state):
            return la.lu_solve(inv, state, overwrite_b=True)

    # Integrate forward in time.
    for j in range(1, k):
        u_nonlin[:, j-1] = nonlin(u[:, j-1], t[j-1])
        u[:, j] = _linsolve(u[:, j-1] + dt*u_nonlin[:, j-1])

    # Return states (and derivatives if desired).
    if compute_du:
        u_nonlin[:, -1] = nonlin(u[:, -1], t[-1])
        du = (A @ u) + u_nonlin
        return u, du
    return u
...
class _EvaluatedFHNROM(_SystemROM):
    ...
    def evaluate(self, t, u_, f):
        """Evaluate the right-hand side of the reduced-order model,
        du / dt = ROM(t, u).
    ...
    def predict(self, u0, t, f, reconstruct=True):
        ...
        def _nonlin(u_, tj):
            return self.nonlin_(tj, u_, f)

        y = imex(t, u0_, self.A_, _nonlin, compute_du=False, sparse_lu=False)
        return (self.Vr @ y) if reconstruct else y
...
class AffineFHNROM(_SystemROM):
    ...
    def _construct_solver(self, bases, params, states, ddts, inputs):
        ...
        self.solvers_ = [
            opinf.lstsq.solver(D1, R1, np.ones(D1.shape[1])),
            opinf.lstsq.solver(D2, R2, 0),  # 1
        ]
    ...
    def _evaluate_solver(self, λs):
        ...
        # P1 = np.full(self.solvers_[0].d, λ1)
        P1 = np.zeros(self.solvers_[0].d)
        P1[-(self._r12 + self._r13):] = λ2
        P1[-self._r13:] = λ3

        # Regularizer for second problem (L2 regularization).
        # P2 = λ1
        P2 = 0

        Ps = (P1, P2)
        Ohats = [solver.predict(P).T for solver, P in zip(self.solvers_, Ps)]
        self._extract_operators(Ohats)
...
class FHNSolver:
    ...
    snapshots : (s, n, k) ndarray
        State snapshots corresponding to each scenario parameter set.
    derivatives : (s, n, k) ndarray
        Time derivatives of state snapshots by scenario parameter set.
    ...
    def full_order_solve(self, params, f):
        ...
        Id = np.eye(self.x.size)
        A = np.block([[A1.toarray(), -Id/ε], [β*Id, -δ*Id]])

        # Define the nonlinear part of the model.
        def nonlin(u, t):
            u1, u2 = np.split(u, 2, axis=0)
            du1dt = (α + 1.1*(u1**2) - u1**3)/ε
            du1dt[0] -= 2*ε*f(t)/self._dx
            du2dt = np.ones_like(u2)*α
            return np.concatenate([du1dt, du2dt], axis=0)

        # Integrate the model in time.
        U, dU = imex(self.t_dense, u0, A, nonlin,
                     compute_du=True, sparse_lu=True)
        return U[:, ::self.downsample], dU[:, ::self.downsample]

    def add_snapshot_set(self, params=None, f=None):
        ...
        with utils.timed_block(f"High-fidelity solve at µ = {params}"):
            snaps, dts = self.full_order_solve(params, f)
    ...
    def add_snapshot_sets(self, params, f=None):
        ...
        for i, µ in enumerate(parameters):
            ...
            with utils.timed_block(f"High-fidelity solve at µ = {µ}"):
                snaps, dts = self.full_order_solve(µ, f)
    ...
    def _subplot_grid():
        ...
        spec = fig.add_gridspec(nrows=2, ncols=2, hspace=.05,
                                width_ratios=[1.5, 1],
                                height_ratios=[1, 1])
...
class FHNROMSolver(FHNSolver):
    ...
    def train_rom(self, rs, reg=None, bases=None,
        ...
        reg : (3,) ndarray
            Regularization hyperparameters λ1, λ2, λ3.
        ...
        regguess : (3,) ndarray or (2,) ndarray or None
        ...
        def training_error_from_rom(log10_λs):
            ...
            with utils.timed_block(f"\nTesting ROM with λ={λs}",
                ...
                if µ_test is not None:
                    u0_ = np.zeros(sum(rs))
                    for µ in µ_test:
                        try:
                            with np.warnings.catch_warnings():
                                np.warnings.simplefilter("ignore")
                                rom.predict(µ, u0_, self.t_dense,
                                            config.fhn_input,
                                            reconstruct=False)
                        except Exception as e:
                ...
                for µ, U_ in zip(self.parameters, Us_):
                    try:
                        with np.warnings.catch_warnings():
                            np.warnings.simplefilter("ignore")
                            U_rom = rom.predict(µ, U_[:, 0], self.t_dense,
                                                config.fhn_input,
                                                reconstruct=False)
                        U_rom = U_rom[:, ::self.downsample]
                        # U_rom = rom.predict(µ, U_[:, 0], self.t,
                        #                     config.fhn_input,
                        #                     reconstruct=False,
                        #                     scipy=True)
                ...
                logging.info(f"STABLE ROM: λs={λs}, "
        ...
        print(f"\n**** GRID SEARCH **** "
              f"({np.prod([len(grid) for grid in gridsearch]):d} trials)")
        errs = {}
        log10_grid2, log10_grid3 = gridsearch
        λ1 = 0
        for ind, (λ2, λ3) in enumerate(itertools.product(log10_grid2,
                                                         log10_grid3)):
            λs = (λ1, λ2, λ3)
